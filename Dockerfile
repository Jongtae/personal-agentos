...
RUN pip install --no-cache-dir . \
    ...
    && mkdir -p /state/data /engine-profile \
    && ln -s /state/data /data \
    && chown -R agentos:agentos /state /engine-profile
ENV PYTHONDONTWRITEBYTECODE=1 \
    PYTHONUNBUFFERED=1 \
    AGENTOS_DATA=/state/data \
    CODEX_HOME=/engine-profile \
    AGENTOS_SUBSCRIPTION_RUNTIME=unsupported-pending-engine-isolation
USER agentos
EXPOSE 8787
