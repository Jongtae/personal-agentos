...
async function refresh(){
 ...
 try{
 const home=await api('/api/home');const state=await api('/api/state');const space=await api('/api/personal-space');const guide=await api('/api/onboarding');const settings=state.settings;const model=settings.model;const tg=settings.telegram;hasModel=home.model_connected;showSubscriptionEngines(settings.subscription_engines);showOnboarding(guide);showWorkspaces(home,state);showPersonalSpace(space);$('home-next-action').textContent=home.next_action;const suggestion=$('workspace-suggestion');suggestion.replaceChildren();if(home.workspace_suggestion){suggestion.hidden=false;suggestion.append(document.createTextNode('이 대화를 작업공간으로 정리할까요? '));const button=element('button','작업공간 만들기');button.type='button';button.onclick=()=>{$('workspace-link').click();$('workspace-title').focus();};suggestion.append(button);}else suggestion.hidden=true;
