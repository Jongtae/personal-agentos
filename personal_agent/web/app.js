...
async function refresh(){
 ...
 try{
 const state=await api('/api/state');const guide=await api('/api/onboarding');const settings=state.settings;const model=settings.model;const tg=settings.telegram;hasModel=!!model.model||!!settings.subscription_engines?.selected;showSubscriptionEngines(settings.subscription_engines);showOnboarding(guide);
 ...
 const engineReady=!!settings.subscription_engines?.selected;$('model-step').textContent=(settings.model_ready||engineReady)?'✓ 모델 또는 구독 엔진 연결':'② 모델 또는 구독 엔진 연결';$('model-step').classList.toggle('done',!!settings.model_ready||engineReady);
