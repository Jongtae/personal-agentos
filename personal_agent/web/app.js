...
async function refresh(){
 ...
 try{
 const home=await api('/api/home');const state=await api('/api/state');const guide=await api('/api/onboarding');const settings=state.settings;const model=settings.model;const tg=settings.telegram;hasModel=home.model_connected;showSubscriptionEngines(settings.subscription_engines);showOnboarding(guide);$('home-next-action').textContent=home.next_action;
