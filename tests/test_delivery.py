...
class DeliveryTests(unittest.TestCase):
    ...
    def test_closed_ux05_state_reconciles_to_next_mp2_selection(self):
        ...
        self.assertEqual(result['next_goal']['id'],'OPERATING-MODE-DEPLOYMENT')
        self.assertEqual(result['next_action'],'await one explicit owner approval before starting the prepared operating deployment')
    ...
    def test_fully_completed_plan_clears_stale_current_iteration_metadata(self):
        ...
        self.assertEqual(self.controller(Runner()).status()['next_action'],'await one explicit owner approval before starting the prepared operating deployment')
