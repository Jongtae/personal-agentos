...
1. **진단과 선택:** doctor를 포함한 checkout에서 `python3 scripts/agentos_doctor.py --root . --candidate COMMIT`를 실행한다. checkout, Docker/Compose 가용성, loopback port, free disk, Compose configuration, secret-free product preflight만 검사한다. container를 시작하거나 state를 수정하지 않고 credential을 읽거나 external service에 접속하지 않는다. `blocked` report는 정확한 안전 next command를 이름으로 제공하며 `deferred_owner_actions`는 failure가 아니다.
