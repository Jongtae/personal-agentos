...
1. **설치와 선택:** TOP-03이 immutable closeout candidate를 기록하기 전에는 지원되는 owner deployment candidate가 없다. `python3 scripts/operating_preflight.py --root .`는 isolated architecture, limited proxy, credential-free product fixture가 통과하면 `ready`를 보고한다. allowlist가 없으면 architecture failure로 취급하지 않고 `provider_egress_allowlist_configured: false`, `provider_egress_reachable: false`, 보류된 owner-configuration gate를 보고한다.
