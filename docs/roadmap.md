...
MP2 is proposed for [conversation-first settings](master-plan-02-proposal.en.md). [D-MP2-01](d-mp2-01-conversation-settings-contract.en.md) is complete on automated documentation evidence: conversation is the default intent surface for safe settings reads and preview/confirm changes, while a local Chrome Settings-like companion remains the manual, inspectable control surface. I-MP2-01 remains inactive until a separate goal-ready issue is created; this adds no runtime implementation or live connection claim.

The owner-local delivery state is reconciled with the merged UX, MP1, and D-MP2-01 records. It must not rerun those historical iterations; its explicit next action is to create the separate goal-ready I-MP2-01 issue before implementation.
