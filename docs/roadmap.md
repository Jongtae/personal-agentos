...
The original owner-approved operating deployment preparation cycle [#258](https://github.com/Jongtae/personal-agentos/pull/258) is historical only: its `v1.0.4` candidate claim was incorrect. [OP-02 / #261](https://github.com/Jongtae/personal-agentos/pull/261) corrects it with immutable commit `18538eabc00a20de64f4c5f5a6ac004aeda2469e`, isolated Codex/profile/owner-state containers, a one-use read-only MCP boundary, default-deny exact-DNS provider proxy, executable preflight, and atomic no-replay recovery fixtures. Automated and CI evidence is complete; Docker build/start, provider allowlist configuration, official login, provider reachability, Telegram activation, and the one owner operating deployment remain explicit operating-mode steps.
