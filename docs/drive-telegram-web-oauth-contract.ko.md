...
소유자 Mac에서만 테스트할 때는 별도 Web client에 `http://localhost:<port>/oauth/google/callback`을 등록할 수 있다. Google은 localhost를 HTTPS 예외로 문서화한다. handoff와 callback 모두 같은 loopback runtime으로 지정하고, 명시적 local-only 모드를 켜면 localhost 이외 주소는 거부한다. 이 Mac의 Telegram Desktop에서만 버튼을 열어 일반 브라우저가 localhost의 AgentOS runtime으로 연결되게 한다. 휴대폰의 localhost는 Mac이 아니라 휴대폰이므로 이 흐름을 사용할 수 없다. 공개 tunnel·DNS·별도 호스팅은 사용하지 않는다.
