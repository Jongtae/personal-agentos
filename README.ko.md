...
버전 1.0.4는 historical이며 지원되는 배포 후보가 아닙니다. Hub v2는 활성 제품 로드맵으로, 구독 연결 실행 엔진, 소유자가 BotFather에서 만든 후 로컬 런타임에 비공개로 페어링한 Telegram 봇, 그리고 옵트인 로컬 컨텍스트 받은편지함을 포함합니다. 향후 지원 후보는 version label 하나가 아니라 current top-level validation이 이름으로 지정한 정확한 commit이어야 합니다. 봇 토큰은 한 번 입력되며 로컬 비공개 연결 저장소에만 보관되고 설정, 이벤트, 내보내기, 로그 및 승인 보고서에서는 제외됩니다. [Hub v2 제품 기반](docs/agentos-hub-v2.ko.md)을 참고하세요.
